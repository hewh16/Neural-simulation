...
for IDSoma=1:size(CellofNeurites,1)
      ...
      for ID1=1:size(PreLenghtVector,1)-1
       ...
       BUMAP2Value=PreBUMAP2Vector(ID1+1,1);
           if LengthValue~=0
              PreNewTubluinVector(1,1)=PreNewTubluinVector(1,1)-1/LengthValue*(PreTubluinVector(1,1)-PreTubluinVector(ID1+1,1))*DiffusionCTubulin*IterationTime-AdvectionCTubulin*(CAssembly*CEnlongation*BUMAP2Value*PreTubluinVector(ID1+1,1)-CDAssembly)^2*PreTubluinVector(ID1+1,1)*IterationTime;
              PreNewTubluinVector(ID1+1,1)=PreTubluinVector(ID1+1,1)+1/LengthValue*(PreTubluinVector(1,1)-PreTubluinVector(ID1+1,1))*DiffusionCTubulin*IterationTime+AdvectionCTubulin*(CAssembly*CEnlongation*BUMAP2Value*PreTubluinVector(ID1+1,1)-CDAssembly)^2*PreTubluinVector(ID1+1,1)*IterationTime-CAssembly*BUMAP2Value*PreTubluinVector(ID1+1,1)*IterationTime-CTubulinDecy*PreTubluinVector(ID1+1,1)*IterationTime;
           end
      end
      %% Modification
      %% Production and Decy for Soma
      ...
      %% Diffuision, Active Transportation and Decy for Soma and Neurites
          if LengthValue~=0
          %% Part One
         OldTubulinVariation=1/LengthValue*(PreTubluinVector(1,1)-PreTubluinVector(ID1+1,1))*DiffusionCTubulin+AdvectionCTubulin*(CAssembly*CEnlongation*BUMAP2Value*PreTubluinVector(ID1+1,1)-CDAssembly)^2*PreTubluinVector(ID1+1,1);
          %% Part Two
         PredictedTubulinVariation=1/LengthValue*(PreNewTubluinVector(1,1)-PreNewTubluinVector(ID1+1,1))*DiffusionCTubulin+AdvectionCTubulin*(CAssembly*CEnlongation*BUMAP2Value*PreNewTubluinVector(ID1+1,1)-CDAssembly)^2*PreNewTubluinVector(ID1+1,1);
         %% Modify
...
for IDSoma=1:size(CellofNeurites,1)
      ...
      for ID1=1:size(PreLenghtVector,1)-1
       ...
       BUMAP2Value=PreBUMAP2Vector(ID1+1,1);
           if LengthValue~=0
              PreNewUMAP2Vector(1,1)=PreNewUMAP2Vector(1,1)-1/LengthValue*(PreUMAP2Vector(1,1)-PreUMAP2Vector(ID1+1,1))*DiffusionCUMAP2*IterationTime-AdvectionCUMAP2*(CAssembly*CEnlongation*BUMAP2Value*PreUMAP2Vector(ID1+1,1)-CDAssembly)^2*PreUMAP2Vector(ID1+1,1)*IterationTime;
              PreNewUMAP2Vector(ID1+1,1)=PreUMAP2Vector(ID1+1,1)+1/LengthValue*(PreUMAP2Vector(1,1)-PreUMAP2Vector(ID1+1,1))*DiffusionCUMAP2*IterationTime+AdvectionCUMAP2*(CAssembly*CEnlongation*BUMAP2Value*PreUMAP2Vector(ID1+1,1)-CDAssembly)^2*PreUMAP2Vector(ID1+1,1)*IterationTime-Beta1*PreUMAP2Vector(ID1+1,1)*IterationTime+Beta2*BUMAP2Value*IterationTime-CUMAP2Decy*PreUMAP2Vector(ID1+1,1)*IterationTime;
           end
      end
      %% Modification
      %% Production and Decy for Soma
      ...
      %% Diffuision, Active Transportation and Decy for Soma and Neurites
          if LengthValue~=0
          %% Part One
         OldUMAP2Variation=1/LengthValue*(PreUMAP2Vector(1,1)-PreUMAP2Vector(ID1+1,1))*DiffusionCUMAP2+AdvectionCUMAP2*(CAssembly*CEnlongation*BUMAP2Value*PreUMAP2Vector(ID1+1,1)-CDAssembly)*PreUMAP2Vector(ID1+1,1);
          %% Part Two
         PredictedUMAP2Variation=1/LengthValue*(PreNewUMAP2Vector(1,1)-PreNewUMAP2Vector(ID1+1,1))*DiffusionCUMAP2+AdvectionCUMAP2*(CAssembly*CEnlongation*BUMAP2Value*PreNewUMAP2Vector(ID1+1,1)-CDAssembly)*PreNewUMAP2Vector(ID1+1,1);
         %% Modify
